...
type RestApiClient interface {
	...
	SendSmsConfirmation(send DoRequest, ctx op_context.Context, resp Response, code string, method string, path string, cmd interface{}, headers ...map[string]string) (Response, error)
}
...
func NewRestApiClientBase(baseUrl string, withBodySender DoRequest, withQuerySender DoRequest) *RestApiClientBase {
	c := &RestApiClientBase{}
	c.BaseUrl = baseUrl
...
func DefaultRestApiClientBase(baseUrl string) *RestApiClientBase {
	return NewRestApiClientBase(baseUrl, DefaultSendWithBody, DefaultSendWithQuery)
}
