...
func LoadObjects(ctx Context, methodName string, filter *db.Filter, objects interface{}, loggerFields ...logger.Fields) error {
...
func FindObject[T common.Object](ctx Context, methodName string, fieldName string, fieldValue interface{}, object T, loggerFields ...logger.Fields) (T, error) {
	c := ctx.TraceInMethod(methodName, loggerFields...)
	defer ctx.TraceOutMethod()

	var nilT T

	found, err := ctx.DB().FindByField(ctx, fieldName, fieldValue, object)
	if err != nil {
...
func UpdateObject(ctx Context, methodName string, obj common.Object, fields db.Fields, loggerFields ...logger.Fields) error {
	c := ctx.TraceInMethod(methodName, loggerFields...)
	defer ctx.TraceOutMethod()
	err := db.Update(ctx.DB(), ctx, obj, fields)
	if err != nil {
		return c.SetError(err)
	}
	return nil
...
func FindUpdateObject[T common.Object](ctx Context, methodName string, fieldName string, fieldValue interface{}, fields db.Fields, object T, loggerFields ...logger.Fields) error {
	c := ctx.TraceInMethod(methodName, loggerFields...)
	defer ctx.TraceOutMethod()

	obj, err := FindObject(ctx, "FindObject", fieldName, fieldValue, object)
	if err != nil {
		return c.SetError(err)
	}

	err = UpdateObject(ctx, "UpdateObject", obj, fields)
	if err != nil {
		return c.SetError(err)
	}
...
func FindObjectByFields[T common.Object](ctx Context, methodName string, fields db.Fields, object T, loggerFields ...logger.Fields) (T, error) {
	c := ctx.TraceInMethod(methodName, loggerFields...)
	defer ctx.TraceOutMethod()

	var nilT T

	found, err := ctx.DB().FindByFields(ctx, fields, object)
	if err != nil {
		return nilT, c.SetError(err)
	}
	if !found {
